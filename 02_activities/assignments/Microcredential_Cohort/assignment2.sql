...
WHERE table_holder.visit_number = 1
...
SELECT
    v.vendor_name,
    p.product_name,
    SUM(vcc.original_price * 5) AS total_revenue
FROM vendor_cross_customer vcc
INNER JOIN vendor  v ON vcc.vendor_id  = v.vendor_id
INNER JOIN product p ON vcc.product_id = p.product_id
