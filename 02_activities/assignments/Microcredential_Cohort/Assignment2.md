...
columns: customer_id; street_address; city; province; postal_code; country

the customer_id is both the primary key and the foreign key linking to the customer table, where each row is a single customer in this customer_addresses table, listing for each customer a single address, which gets overwritten when the customer address is updated/changed. The old address gets removed upon an address update. This creates a minimal storage table, with any inquiries pulling only a single address without needing to filter for the most recent/correct one. However, there happens to be no history of data in this table to lookup past addresses of customers, so tracking historical data (i.e. orders/sales) becomes hard to navigate, with subsequent analysis related to customer_addresses being rewritten each time an address is updated.

Relationship: customer_addresses has a one-to-one relationship with the customer table on customer_id. This is because each customer_id for each customer in the customer table is given only one displayable address in the customer_addresses table.


Architecture 2 - Type 2: retain
table: customer_addresses
columns: customer_id; address_id; street_address; city; province; postal_code; country; valid_from; valid_to; is_active
